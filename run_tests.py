...
#        tempus.tem_lexer.lex_string(matcheroni.Span("case 1 foo \"asdf asdf asdf\""), lex_ctx)
#        self.assertEqual(str(lex_ctx.stack), "[LEX_KEYWORD(case), LEX_INT(1), LEX_IDENT(foo), LEX_STRING(\"asdf asdf asdf\")]")
#
#        lex_ctx  = matcheroni.Context()
#        tempus.tem_lexer.lex_string(matcheroni.Span("++ -- % ^ &"), lex_ctx)
#        self.assertEqual(str(lex_ctx.stack), "[LEX_AFFIX(++), LEX_AFFIX(--), LEX_BINOP(%), LEX_BINOP(^), LEX_BINOP(&)]")
#
#        lex_ctx  = matcheroni.Context()
#        tempus.tem_lexer.lex_string(matcheroni.Span("/*comment*/ 1.0e38 ; 'q' : +="), lex_ctx)
